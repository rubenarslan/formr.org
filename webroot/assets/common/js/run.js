...
(function ($) {
  ...
  RunUnit.prototype.init = function (content) {
    ...
    this.dialog_inputs = this.block.find(
      "div.run_unit_dialog input,div.run_unit_dialog select, div.run_unit_dialog button, div.run_unit_dialog textarea"
    );
    this.description = this.block.find(".run_unit_description");
    this.unit_id = this.dialog_inputs.filter("input[name=unit_id]").val();
    this.run_unit_id = this.dialog_inputs
      .filter("input[name=run_unit_id]")
      .val();
    ...
    this.save_inputs = this.dialog_inputs
      .add(this.position)
    ...
    if (slct.length > 0) {
      if (!slct.select2("container").hasClass("select2-container")) {
        // if it's not already select2ified
  ...
  RunUnit.prototype.changes = function (e) {
    if (!this.unsavedChanges) {
      // dont touch the DOM for every change
  ...
  RunUnit.prototype.test = function (e) {
    ...
    $.ajax({
      ...
      .fail(
        $.proxy(function (e, x, settings, exception) {
      ...
      .done(
        $.proxy(function (data) {
          ...
          if (!data) {
            ...
            $modal = $(
              $.parseHTML(
                getHTMLTemplate("tpl-confirmation", {
                  content:
                    "Are you sure you want to delete this run unit and all it's data?",
                })
              )
            );
            $modal
  ...
  RunUnit.prototype.serialize = function () {
    ...
    for (var i = 0; i < arr.length; i++) {
      if (
        arr[i].name != "unit_id" &&
        arr[i].name != "run_unit_id" &&
        arr[i].name.substr(0, 8) != "position"
      )
  ...
  Run.prototype.exportUnits = function () {
    ...
    for (var i = 0; i < this.units.length; i++) {
      ...
      exportDialog.append(
        $(
          $.parseHTML(
            getHTMLTemplate("tpl-export-unit-block", {
              unit_pos: unit.position,
              unit_json: JSON.stringify(unit, null, "\t"),
            })
          )
        )
      );
    ...
    if (unsavedChanges) {
      bootstrap_modal(
        "Please save all changes before export.",
        "Unsaved Changes"
      );
      return;
    }
    ...
    var $modal = $(
      $.parseHTML(
        getHTMLTemplate("tpl-export-units", {
          run_name: this.name,
      ...
      .on("shown.bs.modal", function () {
        $modal.find(".confirm-export").click(function (e) {
          ...
          if (!export_name || !pattern.test(export_name)) {
            bootstrap_alert(
              "Enter a valid export name",
              "Export name invalid.",
              ".run_export_before_alert"
            );
            return false;
          }
          ...
          if ($.isEmptyObject(selectedUnits)) {
            bootstrap_alert(
              "You need to select at least one unit to export.",
              "Nothing chosen.",
              ".run_export_before_alert"
            );
            return false;
    ...
    if (typeof this.reorder_button.attr("disabled") === "undefined") {
      ...
      $(this.units).each(function (i, elm) {
        ...
        if ($.inArray(pos, are_positions_unique) > -1) {
          bootstrap_alert(
            "You used the position " +
              pos +
              " more than once, therefore the new order could not be saved. <a href='#run_unit_" +
              elm.run_unit_id +
              "'>Click here to scroll to the duplicated position.</a>",
            "Error.",
            ".run_units"
          );
          dupes = true;
        ...
        $.ajax({
          ...
          .done(
            $.proxy(function (data) {
              ...
              var new_order = are_positions_unique
                .sort(function (x, y) {
                  return x - y;
                })
                .join(",");
  ...
  Run.prototype.lock = function (on, context) {
    ...
    var $elements = context.find(
      ".import_run_units, .run_unit_description, .position, .remove_unit_from_run, .reorder_units, .unit_save, .form-control, select, .from_days, .add_run_unit"
    );
  ...
  Run.prototype.panic = function (e) {
    ...
    var $modal = $($.parseHTML(getHTMLTemplate("tpl-confirmation", vars))).attr(
      "id",
      "run-panic-dialog"
    );
    $modal
