...
// Deploy gate (update_formr.sh): fail loudly so the 063 ALTER can't blow up
...
if ($fail_on_blocking && $remaining > 0) {
    fwrite(STDERR, "--fail-on-blocking: {$remaining} tuple(s) still block the UNIQUE key (patch 063); resolve the review clusters above.\n");
    exit(2);
}
exit(0);
