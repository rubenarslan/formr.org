...
- **R-facing `survey_unit_sessions` data no longer fans out for reused units.** `UnitSession::getRunData`'s join matched `survey_run_units` by `unit_id`, so a unit placed at N positions returned every session row N times, in undefined order, with N different `position` values. R code anchored on that data — a Pause `relative_to`, a Branch condition reading `survey_unit_sessions$created`/`$position` — could compute from an arbitrary duplicate; on a live instance this ended a monthly gate Pause ~26 days early. The join now pins each row to its own placement via `run_unit_id` (legacy NULL rows keep the old behaviour). Verified against live repro data: 24 fanned rows → 10 rows, one per session, each with its own position.
- **A failed unit-session INSERT is no longer silent.** `UnitSession::create()` caught every exception, rolled back, and said nothing — while `moveOn()` had already saved the advanced position. The subsequent no-id `load()` then resolved an *arbitrary* same-`unit_id` row (no ORDER BY, cross-placement for reused units). Together these could march a run session across many positions leaving zero session rows (observed live: a same-second 111→151 leap with no intermediate rows). The exception is now logged with unit/placement/session context, and the `load()` fallback resolves the current position's placement first (`run_unit_id`, legacy `unit_id` fallback), newest row first.
- **The "no live session at this position" recovery hop now counts toward `MAX_EXECUTION_COUNT`.** `execute()`'s `!currentUnitSession → moveOn()` recursion bypassed the `executionCount` spam backstop, allowing an unbounded silent position march within a single request; it is now bounded and logged.
