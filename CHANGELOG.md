...
### Fixes
- **Run-engine audit hardening — all 23 confirmed findings** (see `documentation/agent_doc/run_engine_audit_2026-07.md`. `formr-docker/diagnose_run_engine.sh` checks whether an instance was already affected; `formr-docker/rescue_run_engine.sh` remediates existing damage).
  ...
  - **Daemon deadline handling (F4/F15).** The END-q path revalidates the deadline against current state before expiring, so sliding-window surveys re-arm at the fresh `last_active + Z` instead of expiring actively-working participants; deadlines are persisted even on cron-inactive runs so the web overdue guard can act.
