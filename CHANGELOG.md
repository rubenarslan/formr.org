...
- form_v2 R-evaluation redesign:
  ...
  - Bare R (un-wrapped) in `value` is **no longer supported**. Compat scanner classifies as `bare_r`; FormRenderer raises a validation error at render. Admins wrap in `r(...)` to opt into the allowlisted server path.
