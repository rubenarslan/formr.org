...
- **A failed email no longer spins the daemon at full speed.** A send failure correctly returns neither `end_session` nor `move_on` — the participant must not be advanced past an email that never went out — but it also left `queued`/`expires` untouched, and the daemon's pickup poll selects on `expires <= NOW()`, so the row was re-selected on the very next pass (~15 s) and every pass after that, indefinitely. A rate-limited recipient (thresholds are per address, instance-wide: 1/min, 10/day, 60/week — an ESM design or a shared lab address reaches them legitimately), an SMTP outage or a missing recipient therefore produced a hot loop that re-knitted the body through OpenCPU and re-fired `notify_study_admin` on every tick, burning metered compute and mailing the owner until someone noticed. Soft failures — including the "user disabled receiving emails" branch, which had the same dead end — now re-arm with the same backoff knob the Pause/Branch recheck uses (`unit_session.queue_expiration_extension`, default +10 minutes), which also lines up with the admin-notification throttle so the error mail drops to roughly one per retry. Retrying rather than giving up is deliberate: a rate-limit window drains and an outage ends, whereas moving on would silently drop a study's email.
- **The Pause/Branch recheck backoff now measures how long something has been failing, not how long the participant has been waiting.** Audit F19's escalating backoff (10 min → 1 h → 6 h) was keyed on the unit session's age, which conflates "parked for a long time" with "broken for a long time": a longitudinal Pause legitimately waiting days that hit its *first* transient OpenCPU blip was demoted straight to the 6-hour tier, so a timed prompt could be delivered six hours late because of one bad minute — and nothing reset the tier, so the demotion outlived the fault. The tier is now keyed on the length of the current failure streak, which the first successful recheck clears. The streak start lives in the existing `state_log` JSON rather than a new column: it is transient bookkeeping about a state the row is already in, a patch release should not migrate schema, and any normal transition overwrites `state_log` anyway, so a stale marker cannot outlive the session. Regression test `tests/PauseRecheckBackoffTest.php` (red pre-fix on all three tiers).
- **Importing a run bundle no longer silently unlinks every survey from its Google Sheet.** `SurveyStudy::getSettings()` exports `google_file_id`, but the mass-assignment allowlist added to `createFromData` in `987cfba8` deliberately left it out, so a round-trip through export → import dropped the link with no notice and the run editor's resync button then had nothing to resync. It is now on the allowlist. The risk that exclusion was standing in for is real but was never actually closed by it: the value reaches `google_download_survey_sheet()`, which interpolates it into a *destination filename*, so `../` in an id is a write-outside-the-upload-dir primitive — and the admin settings form has passed `google_file_id` through all along. That is now guarded properly, at the shared choke point: `google_get_sheet_id()` validates the id against the base64url shape a Drive file id actually has, which covers the admin form and the import path together. (Host-level SSRF was never reachable — the id lands in the path segment of a fixed `docs.google.com` URL.)
