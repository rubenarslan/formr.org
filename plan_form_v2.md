...
- [x] Upgrade-time compatibility scan — `bin/form_v2_compat_scan.php <study_id|study_name>` reports per-item showif/value classification (empty / r-wrapped / JS-OK / needs r(...) wrap) and prints suggested `r(...)` wrappings for flagged expressions. Heuristic-based — looks for R-only tokens (ifelse/c/tail/paste/is.na/%in%/NA/`<-`/`$`-access) in the Item.php transpile output. Informational, not auto-mutating — admins still decide per-row whether to wrap in `r()`. Auto-wrap (`--auto-wrap` flag mutating `survey_items.showif`) deferred
...
### 13.34 Webpack production mode skips the bundle write when content is unchanged

After `Alpine.data` / `Alpine.directive` edits in `main.js`, `npx webpack --mode production` reported "compiled with 3 warnings in 70s" but `ls -la webroot/assets/build/js/form.bundle.js` showed the SAME mtime as the previous build. The browser then loaded a (legitimately) old cached version on reload, and debugging "why doesn't Alpine initialize?" chased a phantom for two cycles. **mtime lies because webpack 5 does content-hash comparison before writing** — identical output → no disk write → stale mtime. `touch webroot/assets/form/js/main.js && npx webpack` doesn't force a write either if the OUTPUT is byte-identical. The reliable confirmation is `grep "fmrForm\|x-showif" webroot/assets/build/js/form.bundle.js` (or any distinctive string from the source). A hash in the URL (`?v=<timestamp>`) cache-busts the browser; the admin layout already appends one, so a hard reload on the participant page is usually enough once you've confirmed the bundle actually contains your changes.

### 13.35 Programmatic `input.checked = true` on radios bypasses the browser's sibling-uncheck

Under Playwright / `browser_evaluate`, `radio3.checked = true` leaves radio2 ALSO `checked=true` if radio2 was previously selected. The browser's native "radios with the same `name` are mutually exclusive" sync only fires on user-initiated events (click, keyboard navigation); assigning `.checked` in JS doesn't trigger it. `document.querySelector('input[name=X]:checked')` then returns the FIRST match (radio2) even though you meant to select radio3, and `_syncInput`/showif reads the wrong value. In tests: either click the visible `<button data-for>` element via the initButtonGroups wiring (which clears siblings imperatively), or explicitly uncheck all same-name siblings before setting the target (`[...document.querySelectorAll('input[name=X][type=radio]')].forEach(r => r.checked = false); radio3.checked = true; radio3.dispatchEvent(new Event('change',{bubbles:true}))`). Same gotcha applies when using any framework state → DOM sync that assumes a normal browser radio group.
