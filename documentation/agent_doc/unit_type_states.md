...
| **SkipForward** (Branch) | PENDING → directly to **ENDED** | **ENDED** | Evaluates OpenCPU condition. Boolean TRUE → `end_session+run_to=<if_true>`; FALSE → `end_session+move_on`. **Numeric return** → treats the value as an absolute position: `end_session+run_to=<N>` if that position exists, else `end_session+move_on` (continue after the skip) + admin notify. Can cause SUPERSEDED on the `run_to` target's prior row (diagnostic only). |
| **SkipBackward** (Branch) | PENDING → directly to **ENDED** | **ENDED** | Same shape as SkipForward including numeric-position support; back-jump nature means it's the **primary legitimate source** of duplicate-`unit_id` rows in a run-session. The `iteration` column distinguishes loop passes. |
