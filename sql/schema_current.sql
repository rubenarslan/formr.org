...
ALTER TABLE `survey_items`
 ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `study_item` (`study_id`,`name`), ADD KEY `fk_survey_items_survey_studies1_idx` (`study_id`), ADD KEY `type` (`study_id`,`type`);

--
-- Indexes for table `survey_items_display`
--
ALTER TABLE `survey_items_display`
 ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `session_item_views` (`session_id`,`item_id`), ADD KEY `id_idx` (`item_id`), ADD KEY `answered` (`session_id`,`saved`);
...
ALTER TABLE `survey_runs`
 ADD PRIMARY KEY (`id`), ADD KEY `fk_runs_survey_users1_idx` (`user_id`), ADD KEY `fk_survey_runs_survey_units1_idx` (`reminder_email`), ADD KEY `fk_survey_runs_survey_units2_idx` (`service_message`), ADD KEY `fk_survey_runs_survey_units3_idx` (`overview_script`), ADD KEY `fk_survey_runs_survey_units4_idx` (`deactivated_page`);
