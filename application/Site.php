...
class Site {
    ...
    /**
     * Per-request CSP nonce. MUST NOT persist across requests: Site is
     * serialized into the session (see webroot/index.php), so this is reset
     * in updateRequestObject() — which runs on every request — and lazily
     * regenerated by getCspNonce(). A cached nonce would defeat the CSP.
     *
     * @var string|null
     */
    ...
    public function updateRequestObject($path = null) {
        ...
        $this->path = $path;
        // Drop any nonce carried over via the session-serialized Site; the
        // next getCspNonce() call regenerates a fresh per-request value.
        $this->cspNonce = null;
    }

    /**
     * Lazily generate and return this request's CSP nonce (128-bit, base64url).
     *
     * @return string
     */
    public function getCspNonce() {
        if ($this->cspNonce === null) {
            $this->cspNonce = rtrim(strtr(base64_encode(random_bytes(16)), '+/', '-_'), '=');
        }
