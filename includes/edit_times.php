...
function get_edit_times() {
	...
	debug("<strong>get_edit_times:</strong> Array size: " . sizeof($arr) . " items");
    return $arr;
}
...
function update_timestamps($vpncode,$timestarted) {
	...
	debug("<strong>update_timestamps:</strong> $time");
...
function has_entries_for_edit_time($vpncode,$starttime,$endtime) {
    ...
    debug("<strong>has_entries_for_edit_time</strong> 'query is': " . $query_string);
    ...
    if( mysql_num_rows($results) > 0) {
		debug("<strong>has_entries_for_edit_time:</strong> TRUE");
        return true;
    } else {
		debug("<strong>has_entries_for_edit_time:</strong> FALSE");
        return false;
    }
}
...
function can_edit_now($curtime,$times,$vpncode) { # MOOONSTER
    foreach( $times as $time ) {
		if( $time['starttime'] < $time['endtime'] ) { //everything is easy
			debug("can_edit_now, normal case");
			...
			if(( $curtime > $start AND $curtime < $end ) || ( $timestarted > $start AND $timestarted < $end )) {
				...
				debug("<strong>can_edit_now:</strong> TRUE");
	            return true;
	        }
			
		} else {
				debug("can_edit_now, special case");
			...
			debug("<strong>can_edit_now:</strong> start1 ".$start1 ."-end1 ".$end1."start2 ".$start2 ."-end2 ".$end2);
	...
	debug("<strong>can_edit_now:</strong> FALSE");
    return false;
}
...
function get_entry_for_time($vpncode,$starttime,$endtime) {
	if(TIMEDMODE)
	    ...
	    $query_string = "SELECT id FROM ".RESULTSTABLE." WHERE vpncode='$vpncode'";
    debug( "<strong>get_entry_for_time:</strong> says ''query is': " . $query_string);
...
function get_timestarted($vpncode,$starttime,$endtime) {
	...
	debug("<strong>get_timestarted:</strong> vpncode => " . $vpncode);
	debug("<strong>get_timestarted:</strong> starttime => " . $starttime);
	debug("<strong>get_timestarted:</strong> endtime => " . $endtime);
	...
	if( empty($entry["timestarted"]) ) {
		debug("<strong>get_timestarted:</strong> didn'nt find any timestarted for this edit slot");
		return time();
	} else {
		debug("<strong>get_timestarted:</strong> using timestarted found in db");
